...
describe("Executable", function(this: Mocha.ISuiteCallbackContext) {
    ...
    describe("Configuration file", () => {
        ...
        it("Works for config file above cwd", done => {
            const dir = path.join(__dirname, "..", "..", "..", "test", "config", "findup");
            execCli(["a.ts"], { cwd: path.join(dir, "dir0") }, (err, stdout, stderr) => {
                assert.isNull(err);
                assert.equal(stdout, "");
                assert.equal(stderr, "");
                done();
        ...
        it("Warns for config file with bad case", done => {
            const cwd = path.join(__dirname, "..", "..", "..", "test", "config", "findup", "dir1");
            execCli(["a.ts"], { cwd }, (err, stdout, stderr) => {
                assert.isNull(err);
                assert.equal(stdout, "");
                assert.equal(
                    stderr,
                    `Using mixed case tslint.json is deprecated. Found: ${path.join(
                        cwd,
                        "Tslint.json"
                    )}`
                );
    ...
    describe("--project flag", () => {
        ...
        it("warns if file-to-lint does not exist", async () => {
            ...
            assert.include(
                result.stderr,
                `${path.normalize("test/files/tsconfig-test/non-existent.test.ts")}' does not exist`
            );
        });
...
async function execRunnerWithOutput(options: Partial<Options>) {
    let stdout = "";
    let stderr = "";
    const status = await execRunner(options, {
        log(text) {
            stdout += text;
        },
        error(text) {
...
async function execRunner(options: Partial<Options>, logger: Logger = dummyLogger) {
